...
char my_argv2[CONFIG_FILE_LINE_BUFFER_SIZE+1];
...
LaneDetectorParser_configfile (char * const filename, struct LaneDetectorParserInfo *args_info, int override, int initialize, int check_required)
{
  ...
  while ((fgets(linebuf, CONFIG_FILE_LINE_SIZE, file)) != NULL)
    ...
    noarg:
      ...
      strcat (my_argv2, len > 1 ? "--" : "-");
      strcat (my_argv2, fopt);
      if (len > 1 && ((farg &&*farg) || equal))
          strcat (my_argv2, "=");
      if (farg && *farg)
          strcat (my_argv2, farg);

      cmd_line_list_tmp = (struct line_list *) malloc (sizeof (struct line_list));
      cmd_line_list_tmp->next = cmd_line_list;
      cmd_line_list = cmd_line_list_tmp;
      cmd_line_list->string_arg = gengetopt_strdup(my_argv2);
    } /* while */
